...
class DataGenerator:
    ...
    def generate(
        ...
        # CIFAR100 dataset, provided through Keras
    ...
    def mask_images(self, mask_size=8):
        """
        Mask the center region of each image with a square of zeros.
        :param mask_size: Size of the square mask.
        """
        self.masked_x_train = self.x_train.copy()
        self.masked_x_valid = self.x_valid.copy()
        self.masked_x_test = self.x_test.copy()

        def mask_center(img):
            h, w = img.shape[:2]
            start = h // 2 - mask_size // 2
            img[start:start + mask_size, start:start + mask_size] = -1  # Mask region with -1
            return img

        for i in range(self.masked_x_train.shape[0]):
            self.masked_x_train[i] = mask_center(self.masked_x_train[i])
        for i in range(self.masked_x_valid.shape[0]):
            self.masked_x_valid[i] = mask_center(self.masked_x_valid[i])
        for i in range(self.masked_x_test.shape[0]):
            self.masked_x_test[i] = mask_center(self.masked_x_test[i])
